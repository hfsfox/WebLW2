...
//return parameters from local storage
function getLS(key) {
  return JSON.parse(localStorage.getItem(key));
}

// get list fom API and save in local storage
function updateLS() {
  fetch(API)
    .then(response => {
      ...
      const promises = data.results.map(pokemon => {
        return fetch(pokemon.url)
          ...
          .then(data => {
            return {
              name: pokemon.name,
              pokePic: data.sprites.front_default, //picture
              height: data.height,
              weight: data.weight,
